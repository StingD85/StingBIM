...
namespace StingBIM.Data.Materials
{
    ...
    public class MaterialApplicator
    {
        ...
        public MaterialApplicationResult ApplyToElement(Element element, string materialCode)
        {
            ...
            {
                StingBIMLogger.Error($"Error applying material to element {element.Id}: {ex.Message}", ex);
        ...
        public MaterialApplicationResult ApplyBatch(IEnumerable<Element> elements, string materialCode)
        {
            ...
            try
            {
                ...
                StingBIMLogger.Info($"Starting batch material application to {elementList.Count} elements...");
                ...
                using (var trans = new Transaction(_document, "Apply Materials"))
                {
                    ...
                    try
                    {
                        ...
                        StingBIMLogger.Info($"Batch complete: {successCount} succeeded, {failureCount} failed");
            ...
            {
                StingBIMLogger.Error($"Batch material application failed: {ex.Message}", ex);
                return MaterialApplicationResult.Failure(ex.Message);
            }
        }
            ...
            Func<Element, string> materialSelector)
        {
            ...
            {
                var elementList = elements.ToList();
                StingBIMLogger.Info($"Starting mapped batch material application to {elementList.Count} elements...");
                ...
                using (var trans = new Transaction(_document, "Apply Materials (Mapped)"))
                {
                    ...
                    try
                    {
                        ...
                        StingBIMLogger.Info($"Mapped batch complete: {successCount} succeeded, {failureCount} failed");
            ...
            {
                StingBIMLogger.Error($"Mapped batch material application failed: {ex.Message}", ex);
                return MaterialApplicationResult.Failure(ex.Message);
            }
        }
        ...
        public MaterialApplicationResult ApplyToCategory(BuiltInCategory category, string materialCode)
        {
            try
            {
                ...
                StingBIMLogger.Info($"Applying material to {elements.Count} elements in category {category}");

                return ApplyBatch(elements, materialCode);
            }
            catch (Exception ex)
            {
                StingBIMLogger.Error($"Failed to apply material to category: {ex.Message}", ex);
                return MaterialApplicationResult.Failure(ex.Message);
            }
        }
        ...
        public MaterialApplicationResult PaintElement(Element element, string materialCode, int faceIndex = -1)
        {
            ...
            {
                StingBIMLogger.Error($"Failed to paint element: {ex.Message}", ex);
                return MaterialApplicationResult.Failure(ex.Message);
            }
        }
        ...
        private Material GetOrCreateMaterial(MaterialDefinition materialDef)
        {
            try
            {
                ...
                if (_options.CreateMaterialsIfMissing)
                {
                    using (var trans = new Transaction(_document, "Create Material"))
                    {
                        ...
                        try
                        {
                            var newMaterial = Material.Create(_document, materialDef.Name);
                            ...
                            StingBIMLogger.Debug($"Created new material: {materialDef.Name}");
            ...
            {
                StingBIMLogger.Error($"Error creating material '{materialDef.Name}': {ex.Message}", ex);
                return null;
            }
        }
        ...
        private void SetMaterialProperties(Material material, MaterialDefinition materialDef)
        {
            ...
            {
                StingBIMLogger.Warning($"Failed to set properties for material '{materialDef.Name}': {ex.Message}");
            }
        }
        ...
        private bool ApplyMaterialToElement(Element element, Material material)
        {
            ...
            {
                StingBIMLogger.Debug($"Failed to apply material to element {element.Id}: {ex.Message}");
                return false;
            }
        }
