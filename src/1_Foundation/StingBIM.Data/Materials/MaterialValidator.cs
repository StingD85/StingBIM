    ...
    public class MaterialValidator
    {
        ...
        public MaterialValidationResult ValidateElement(Element element)
        {
            ...
            try
            {
                ...
                StingBIMLogger.Debug($"Validated element {element.Id}: {result.Errors.Count} errors, {result.Warnings.Count} warnings");

                return result;
            }
            catch (Exception ex)
            {
                StingBIMLogger.Error($"Error validating element {element.Id}: {ex.Message}", ex);
        ...
        public MaterialBatchValidationResult ValidateBatch(IEnumerable<Element> elements)
        {
            ...
            try
            {
                ...
                StingBIMLogger.Info($"Starting batch validation of {elementList.Count} elements...");
                ...
                foreach (var element in elementList)
                {
                    ...
                    {
                        StingBIMLogger.Warning($"Failed to validate element {element.Id}: {ex.Message}");
                        batchResult.InvalidCount++;
                    }
                }

                StingBIMLogger.Info($"Batch validation complete: {batchResult.ValidCount} valid, {batchResult.InvalidCount} invalid");

                return batchResult;
            }
            catch (Exception ex)
            {
                StingBIMLogger.Error($"Batch validation failed: {ex.Message}", ex);
                throw;
            }
        }
        ...
        public MaterialBatchValidationResult ValidateCategory(BuiltInCategory category)
        {
            try
            {
                ...
                StingBIMLogger.Info($"Validating {elements.Count} elements in category {category}");

                return ValidateBatch(elements);
            }
            catch (Exception ex)
            {
                StingBIMLogger.Error($"Failed to validate category: {ex.Message}", ex);
                throw;
            }
        }
        ...
        public void AddRule(ValidationRule rule)
        {
            ...
            _rules.Add(rule);
            StingBIMLogger.Debug($"Added validation rule: {rule.Name}");
        }
        ...
        public bool RemoveRule(string ruleName)
        {
            ...
            {
                _rules.Remove(rule);
                StingBIMLogger.Debug($"Removed validation rule: {ruleName}");
                return true;
            }
        ...
        {
            _rules.Clear();
            StingBIMLogger.Debug("Cleared all validation rules");
        }
        ...
        public MaterialValidationSummary GetValidationSummary()
        {
            ...
            {
                StingBIMLogger.Error($"Failed to generate validation summary: {ex.Message}", ex);
                throw;
            }
        }
