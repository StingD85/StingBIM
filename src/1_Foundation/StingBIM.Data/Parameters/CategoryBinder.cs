        ...
        private bool ApplyParameterBinding(ParameterDefinition paramDef, List<CategoryBinding> bindings)
        {
            try
            {
                ...
                if (definition == null)
                {
                    ...
                    var options = new ExternalDefinitionCreationOptions(paramDef.Name, paramDef.RevitParameterType)
                    {
                        GUID = paramDef.Guid,
                ...
                if (bindingMap.Contains(definition))
                {
                    ...
                    bindingMap.ReInsert(definition, newBinding, BuiltInParameterGroup.PG_IDENTITY_DATA);
                }
                else
                {
                    // Insert new binding
                    bindingMap.Insert(definition, newBinding, BuiltInParameterGroup.PG_IDENTITY_DATA);
                }
