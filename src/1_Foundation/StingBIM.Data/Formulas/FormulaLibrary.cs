...
namespace StingBIM.Data.Formulas
{
    ...
    public class FormulaLibrary
    {
        ...
        public async Task<int> LoadFromCsvAsync(string csvFilePath)
        {
            ...
            try
            {
                StingBIMLogger.Info($"Loading formulas from {Path.GetFileName(csvFilePath)}...");
                ...
                StingBIMLogger.Info($"Formula library loaded: {Count} formulas");
                return Count;
            }
            catch (Exception ex)
            {
                StingBIMLogger.Error($"Failed to load formula library: {ex.Message}", ex);
                throw;
            }
        }
        ...
        public void Clear()
        {
            lock (_lock)
            {
                ...
                StingBIMLogger.Debug("Formula library cleared");
            }
        }
        ...
        private List<FormulaDefinition> ParseCsvFile(string csvFilePath)
        {
            ...
            try
            {
                using (var reader = new StreamReader(csvFilePath))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        {
                            StingBIMLogger.Warning($"Failed to parse line {lineNumber}: {ex.Message}");
                        }
                    }
                }
            ...
            {
                StingBIMLogger.Error($"Error parsing CSV file: {ex.Message}", ex);
                throw;
            }
        }
        ...
        private FormulaDefinition CreateFormulaFromCsvRow(List<string> values, Dictionary<string, int> columnMap)
        {
            ...
            {
                StingBIMLogger.Warning($"Failed to create formula from CSV row: {ex.Message}");
                return null;
            }
        }
