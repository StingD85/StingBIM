    ...
    public class ScheduleGenerator
    {
                ...
                foreach (var fieldDef in fields.OrderBy(f => f.DisplayOrder))
                {
                    try
                    {
                        ...
                        if (schedulableField != null)
                        {
                            ...
                            if (fieldDef.Width > 0)
                            {
                                scheduleField.Width = fieldDef.Width / 304.8; // Convert mm to feet
                            }
        ...
        private void ApplyFormatting(ViewSchedule schedule, ScheduleFormatting formatting)
        {
            try
            {
                ...
                {
                    _logger.Debug($"Header color: R{formatting.HeaderColor.Red} G{formatting.HeaderColor.Green} B{formatting.HeaderColor.Blue}");
                }
            ...
            try
            {
                ...
                for (int i = 0; i < fieldCount; i++)
                {
                    ...
                    var schedulableField = definition.GetSchedulableFields()
                        .FirstOrDefault(sf => sf.FieldId == field.FieldId);
                    
                    if (schedulableField != null)
                    {
                        var fieldName = schedulableField.GetName(_document);
                        if (fieldName.Equals(parameterName, StringComparison.OrdinalIgnoreCase))
                        {
                            return field;
                        }
        ...
        /// </summary>
        private ScheduleFilterType ConvertFilterType(Data.Schedules.ScheduleFilterType filterType)
        {
            switch (filterType)
            {
